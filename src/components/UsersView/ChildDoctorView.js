...
function ChildDoctorView() {
    ...
    const fetchDoctors = async () => {
        try {
            const response = await fetch(`https://15.206.80.235:9000/ChildDoctorList`);
            if (response.ok) {
    ...
    const fetchTodaysAppointments = async () => {
        try {
            if (userRole === 'Child') { // Assuming 'child' is the role you want to check
                const response = await axios.get(`https://15.206.80.235:9000/ChildTodaysAppointments?user_id=${child_user_id}`);
                ...
                const response = await axios.get(`https://15.206.80.235:9000/ParentViewAppointments?parent_user_id=${child_user_id}&date=${currentDate}`);
    ...
    const fetchDoctorDetails = async (doctorId) => {
        try {
            const response = await axios.get(`https://15.206.80.235:9000/editdoctorslist/${doctorId}`);
            if (response.status === 200) {
                                                                            ...
                                                                            <button className="btn w-100 btn-outline-danger" onClick={async () => {
                                                                                ...
                                                                                if (doctorDetails) {
                                                                                    ...
                                                                                    if (confirmed) {
                                                                                        ...
                                                                                        const response = await axios.delete(`https://15.206.80.235:9000/cancelappointment/${appointment.doctor_id}/${appointment.parent_id}/${appointment.formatted_date}`);
                                                                                        if (response.status === 200) {
