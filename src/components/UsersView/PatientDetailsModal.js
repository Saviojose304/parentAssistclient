...
function PatientDetailsModal({ show, onHide, parent_id, userId }) {
    ...
    const fetchPatientDetails = async () => {
        try {
            const response = await axios.get(`https://15.206.80.235:9000/getPatientDetailsDoctorView?parent_id=${parent_id}&user_id=${userId}`);
    ...
    const fetchMedicineDetails = async () => {
        try {
            const response = await axios.get(`https://15.206.80.235:9000/getMedicineDetailsDoctorView?parent_id=${parent_id}&user_id=${userId}`);
            if (response.status === 200) {
                                ...
                                <div className='col-md-6'>
                                    ...
                                    <p>
                                        ...
                                        {details.past_surgeries === "No" ? (
                                            ...
                                            <a href={`https://15.206.80.235:9000/${details.past_surgeries}`} target="_blank" rel="noopener noreferrer" className="btn btn-success mx-2 w-20 mt-3">
                                    ...
                                    <p>
                                        ...
                                        {details.text_result === "No" ? (
                                            ...
                                            <a href={`https://15.206.80.235:9000/${details.test_result}`} target="_blank" rel="noopener noreferrer" className="btn btn-success mx-2 w-20 mt-3">
                                                <i className="bi bi-file-arrow-down-fill"></i>
                                            </a>
                                        )}
                                    </p>
